...
const initialize = () => {
    // routeElement.value = rootURL + 'comments';
    // routeElement.focus();
    populateEndpoints();
};
    ...
    fetch(rootURL + 'db')
        ...
        .then(data => {
            ...
            for (endpoint in data) {
                ...
                options.push(`<option value="${rootURL}${endpoint}">${rootURL}${endpoint}</option>`);
                for (item of data[endpoint]) {
                    options.push(`
                        <option value="${rootURL}${endpoint}/${item.id}">
...
const displayCode = () => {
    codeContainer.innerHTML = `fetch('${getEndpoint()}')
    .then(response => response.json())
    .then(data => {
        console.log(data);
    });`;
...
menuElement.onchange = issueGetRequest;
